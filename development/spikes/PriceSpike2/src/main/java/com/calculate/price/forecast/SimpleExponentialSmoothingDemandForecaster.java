...
*/
    ...
    public List<DataFrameVO> forecast(String dataIdentifier, List<DataFrameVO> historicalData, int forecastSize) {
        List<Double> historicalDataList = historicalData.stream().map(hd->hd.getValue()).collect(Collectors.toList());
...
//            System.out.println("SES coefficient alpha:" + forecaster.getAlpha());
          ...
          //  System.out.println("SEMA$$$$$$$$$$$$$$$$Predicted value:" + predictionsSet.get(predictionsSet.size() - 1).findPrecisePrediction());
            ...
            return buildForecastedDataFrames(dataIdentifier,historicalData,resultSet);

        } else {
            //TODO : Handle NPE
            return nextForecaster == null ? null
                    : nextForecaster.forecast(dataIdentifier, historicalData,forecastSize);
        }
    }
