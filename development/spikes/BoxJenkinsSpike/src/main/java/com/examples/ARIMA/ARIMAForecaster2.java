...
public class ARIMAForecaster2 {
    public static void  main(String[] args) throws FileNotFoundException,IOException{
        BufferedReader fileReader= new BufferedReader(new InputStreamReader(
                new FileInputStream(
                        new File("/Users/kulkarnm/app/affaince/development/spikes/BoxJenkinsSpike/data/demands2.tsv"))));
        double[] values= fileReader.lines().mapToDouble(n->Double.parseDouble(n)).toArray();
        Vector ts = Vectors.dense(values);
        ARIMAModel arimaModel = ARIMA.autoFit(ts,1,0,1); //.fitModel(1, 0, 1,ts,true,"css-cgd",null);
