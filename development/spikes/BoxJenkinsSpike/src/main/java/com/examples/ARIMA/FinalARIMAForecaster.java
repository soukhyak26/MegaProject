...
import com.examples.config.DataFrameVO;
import com.examples.config.HistoryMaxSizeConstraints;
import com.examples.config.HistoryMinSizeConstraints;
...
public class FinalARIMAForecaster {
    ...
    public List<DataFrameVO> forecast(String dataIdentifier, List<DataFrameVO> dataFrames) {
        ...
        if (dataFrames.size() > historyMinSizeConstraints.getArima()) {
            ...
            JavaTimeSeriesRDD<String> df = filled.mapSeries(vector -> {
                ...
                ARIMAModel arimaModel = ARIMA.fitModel(1, 0, 0, vector.asBreeze(), true, "css-cgd", null);
                breeze.linalg.Vector<Object> forecasted = arimaModel.forecast(vector.asBreeze(), dataFrames.size() / 2);
