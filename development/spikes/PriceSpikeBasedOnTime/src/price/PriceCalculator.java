...
public class PriceCalculator {
    ...
    public DoublyLinkedList<PriceInput> calculatePrice(PriceInput markPrice, DoublyLinkedList<PriceInput> priceInputs) {
        ...
        while (iterator.hasNext()) {
            ...
            if (null == minusOneInput && null == minusTwoInput && tempInput.getOfferedPrice() < markPrice.getOfferedPrice()) {
                ...
                tempInput.setTrendDeviation(0.1);
                ...
                tempInput.setProfit(profit);
                tempInput.setRevenue(revenue);
                minusOneInput.setDemandTrend(calculateDemandTrend(traversedInputs));
                tempInput.setTrendDeviation(calculateTrendDeviation(minusOneInput, minusTwoInput));
            } else if (null != minusOneInput && null != minusTwoInput && minusOneInput.getProfit() > minusTwoInput.getProfit()
                    && minusOneInput.getQuantity() < minusTwoInput.getQuantity() && minusOneInput.getOfferedPrice() > minusTwoInput.getOfferedPrice()) {
                double y2 = minusOneInput.getOfferedPrice();
                ...
                //double slope=minusOneInput.getSlope() - (minusOneInput.getSlope()*minusOneInput.getDemandTrend()/100);
                double slope = calculateSlope(y2, y1, x2, x1);
                double intercept = minusOneInput.getMRP();
                double offeredPrice = calculateOfferedPrice(intercept, slope, tempInput.getQuantity());
                tempInput.setOfferedPrice(offeredPrice);
                ...
                if (minusOneInput.getQuantity() < minusTwoInput.getQuantity()) {
                    ...
                    double slope = minusOneInput.getSlope() - (minusOneInput.getSlope() * minusOneInput.getDemandTrend());
                    double intercept = minusOneInput.getMRP();
                    double offeredPrice = calculateOfferedPrice(intercept, slope, tempInput.getQuantity());
                    tempInput.setOfferedPrice(offeredPrice);
                    double revenue = calculateRevenue(tempInput.getOfferedPrice(), tempInput.getQuantity());
                ...
                tempInput.setProfit(profit);
                tempInput.setRevenue(revenue);
                tempInput.setDemandTrend(calculateDemandTrend(traversedInputs));
                if (minusTwoInput != null) {
                    minusOneInput.setDemandTrend(calculateDemandTrend(traversedInputs));
                    tempInput.setTrendDeviation(calculateTrendDeviation(minusOneInput, minusTwoInput));
                } else {
                    double trendDeviation = ((minusOneInput.getQuantity()/minusOneInput.getDuration())-(markPrice.getQuantity()/markPrice.getDuration()))/(markPrice.getQuantity()/markPrice.getDuration());
                    tempInput.setTrendDeviation(trendDeviation);
                }
            }
    ...
    private double calculateDemandTrend(List<PriceInput> traversedInputs) {
        double totalDuration = 0;
        double totalQuantity = 0;
        for (PriceInput tempInput : traversedInputs) {
            totalDuration += tempInput.getDuration();
            totalQuantity += tempInput.getQuantity();
        }
        totalDuration = totalDuration - traversedInputs.get(traversedInputs.size()-1).getDuration();
        totalQuantity = totalQuantity - traversedInputs.get(traversedInputs.size()-1).getQuantity();
        double trend = totalQuantity / totalDuration;
        return trend;
    }

    private double calculateTrendDeviation (PriceInput minusOneInput, PriceInput minusTwoInput) {
       // System.out.println("Trend Deviation: " + (minusOneInput.getDemandTrend() - minusTwoInput.getDemandTrend())/minusTwoInput.getDemandTrend());
        return (minusOneInput.getDemandTrend() - minusTwoInput.getDemandTrend())/minusTwoInput.getDemandTrend();
    }
