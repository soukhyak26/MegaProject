...
public class TradingAccountDatabaseSimulator {
    ...
    public static TradingAccount searchActiveLedgerAccountsByAccountIdAndAccountIdentifier(String merchantId, String accountId, AccountIdentifier accountIdentifier) {
        return allTradingAccounts.stream().filter(account->account.getMerchantId().equals(merchantId) && account.getAccountIdentifier()==accountIdentifier && account.getAccountId().equals(accountId) && account.getClosureDate().equals(new LocalDateTime(9999,12,31,23,59,59))).findAny().orElse(null);
    }
    ...
    public static List<TradingAccount> getAllActiveAccounts(String merchantId){
        return allTradingAccounts.stream().filter(ac->ac.getMerchantId().equals(merchantId) && ac.getClosureDate().equals(new LocalDateTime(9999, 12, 31,23,59,59))).collect(Collectors.toList());
    }

    public static TradingAccount getLatestClosedAccount(String merchantId,String accountId){
        return allTradingAccounts.stream().filter(ac->ac.getMerchantId().equals(merchantId) &&
    ...
    public static List<TradingAccount> getAllLatestClosedAccounts(String merchantId){
        return allTradingAccounts.stream().filter(ac->ac.getMerchantId().equals(merchantId) &&
                ac.getLatestVersion()== true &&
