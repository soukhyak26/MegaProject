...
public class SignafloArimaForecaster4 {
    public static void main(String[] args) {
        SignafloArimaForecaster4 forecaster = new SignafloArimaForecaster4();
        List<DataFrameVO> inputs = forecaster.createObservations();
        ...
        TimePeriod timePeriod = forecaster.determineTimePeriod(firstInstanceDate, secondInstanceDate);

        List<OffsetDateTime> offsetDateTimes = forecaster.createOffSetDateTimes(inputs);
        TimeSeries ts = TimeSeries.from(timePeriod, offsetDateTimes,doubleSeries);
        ...
        int forecastHorizon = inputs.size();  // Number of future time points to forecast
        Forecast forecastValues = arimaEstimation.forecast(forecastHorizon);

        // Print forecasted values
        System.out.println("Forecasted Values: " + forecastValues);

        List<DataFrameVO> outputFrames = forecaster.buildForecastedDataFrames("demand",forecastValues );
        for(DataFrameVO vo : outputFrames){
            System.out.println(vo);
        }
    }
    List<DataFrameVO> buildForecastedDataFrames(String identifier, Forecast forecast){
    ...
    public List<OffsetDateTime> createOffSetDateTimes(List<DataFrameVO> observations){
    ...
    private TimePeriod determineTimePeriod(LocalDate firstInstanceDate, LocalDate secondInstanceDate) {
