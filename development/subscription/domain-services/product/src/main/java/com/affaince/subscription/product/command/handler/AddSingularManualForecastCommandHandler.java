...
public class AddSingularManualForecastCommandHandler {
    ...
    public void handle(AddSingularManualForecastCommand command){
       ...
       // product.registerSingularManualForecast(command.getFromDate(),command.getEndDate(),command.getPurchasePricePerUnit(),command.getMrp(),command.getNumberOfNewSubscriptions(),command.getNumberOfChurnedSubscriptions(),command.getForecastContentStatus());
    }
}
