...
When method get
Then status 200
And match response == read('classpath:domains/payments/makepayment2/read-payment-deliverycostaccount.json')
...
When method get
Then status 200
And match response == read('classpath:domains/payments/makepayment2/read-totalreceivedcost.json')
...
When method get
Then status 200
And match response == read('classpath:domains/payments/makepayment2/read-totalreceivablecost.json')
...
When method get
Then status 200
And match response == read('classpath:domains/payments/makepayment2/read-paymentinstallments.json')
